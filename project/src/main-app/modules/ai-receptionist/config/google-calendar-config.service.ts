...
export class GoogleCalendarConfigService {
  ...
  getGoogleCalendarConfig(): { google: any } | undefined {
    ...
    // this.logger.log('serviceAccountJson: ' + JSON.stringify(serviceAccountJson));
    ...
    if (serviceAccountJson) {
      try {
        ...
        this.logger.warn(
          'Failed to parse GOOGLE_SERVICE_ACCOUNT_JSON, using OAuth2 credentials instead',
        );
      }
    }
    ...
    if (!credentials && apiKey) {
      ...
      if (apiKey.length >= 20) {
        ...
        this.logger.warn(
          `Google API key is too short (${apiKey.length} chars, minimum 20). Ignoring API key.`,
        );
      }
    }
