...
describe("ChatApi", () => {
  ...
  describe("sendMessage", () => {
    it("should call send message endpoint with correct data", async () => {
      ...
      const mockResponse = {
        ...
        aiMessage: {
          ...
          leadId: 1,
          message:
            "Thank you for your interest! I would be happy to help you learn more about our services.",
    ...
    it("should handle send message error", async () => {
      ...
      await expect(chatApi.sendMessage(messageData)).rejects.toThrow(
        "Lead not found"
      );
      expect(mockPost).toHaveBeenCalledWith("/chat/send", messageData);
    });
  ...
  describe("getChatHistory", () => {
    ...
    it("should handle get chat history error", async () => {
      ...
      await expect(chatApi.getChatHistory(999)).rejects.toThrow(
        "Lead not found"
      );
  ...
  describe("getChatHistoryByDateRange", () => {
    it("should call get chat history by date range endpoint", async () => {
      ...
      const result = await chatApi.getChatHistoryByDateRange(
        leadId,
        startDate,
        endDate
      );
      ...
      expect(mockGet).toHaveBeenCalledWith(
        "/chat/messages/1?startDate=2024-01-01&endDate=2024-01-31"
      );
      expect(result).toEqual(mockMessages);
    });
    ...
    it("should handle get chat history by date range error", async () => {
      ...
      await expect(
        chatApi.getChatHistoryByDateRange(leadId, startDate, endDate)
  ...
  describe("markMessageAsRead", () => {
    ...
    it("should handle mark message as read error", async () => {
      ...
      await expect(chatApi.markMessageAsRead(messageId)).rejects.toThrow(
        "Message not found"
      );
  ...
  describe("deleteMessage", () => {
    ...
    it("should handle delete message error", async () => {
      ...
      await expect(chatApi.deleteMessage(messageId)).rejects.toThrow(
        "Message not found"
      );
  ...
  describe("getUnreadMessagesCount", () => {
    ...
    it("should handle get unread messages count error", async () => {
      ...
      await expect(chatApi.getUnreadMessagesCount(leadId)).rejects.toThrow(
        "Lead not found"
      );
  ...
  describe("markAllAsRead", () => {
    ...
    it("should handle mark all as read error", async () => {
      ...
      await expect(chatApi.markAllAsRead(leadId)).rejects.toThrow(
        "Lead not found"
      );
  ...
  describe("clearChatHistory", () => {
    ...
    it("should handle clear chat history error", async () => {
      ...
      await expect(chatApi.clearChatHistory(leadId)).rejects.toThrow(
        "Lead not found"
      );
