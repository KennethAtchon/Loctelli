...
export const Route = createFileRoute('/admin/content/forms/$id/edit')({
...
function EditFormTemplatePage() {
  ...
  const loadTemplate = async () => {
    try {
      ...
      navigate({ to: '/admin/content/forms' });
    } finally {
  ...
  const importSchemaFromJSON = () => {
    try {
      ...
      toast.success(`Imported ${schemaWithUniqueIds.length} form fields successfully`);
    } catch (error: any) {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          ...
          <Button onClick={() => navigate({ to: '/admin/content/forms' })} className="mt-4">
            Back to Forms
          </Button>
        </div>
      </div>
