...
describe('LeadsApi', () => {
  ...
  describe('getLeads', () => {
    ...
    it('should handle get leads error', async () => {
      ...
      await expect(leadsApi.getLeads()).rejects.toThrow('Failed to fetch leads')
      expect(mockGet).toHaveBeenCalledWith('/lead')
    })
  })
  ...
  describe('createLead', () => {
    ...
    it('should handle create lead error', async () => {
      ...
      await expect(leadsApi.createLead(createLeadData)).rejects.toThrow('Invalid strategy')
      expect(mockPost).toHaveBeenCalledWith('/lead', createLeadData)
    })
  ...
  describe('updateLead', () => {
    ...
    it('should handle update lead error', async () => {
      ...
      await expect(leadsApi.updateLead(1, updateData)).rejects.toThrow('Invalid status value')
      expect(mockPatch).toHaveBeenCalledWith('/lead/1', updateData)
    })
  })
  ...
  describe('getLeadsByUser', () => {
    ...
    it('should handle get leads by user error', async () => {
      ...
      await expect(leadsApi.getLeadsByUser(999)).rejects.toThrow('User not found')
  ...
  describe('getLeadsByStrategy', () => {
    ...
    it('should handle get leads by strategy error', async () => {
      ...
      await expect(leadsApi.getLeadsByStrategy(999)).rejects.toThrow('Strategy not found')
