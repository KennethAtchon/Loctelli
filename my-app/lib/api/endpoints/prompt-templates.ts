...
export class PromptTemplatesApi {
  ...
  async getAllForSubAccount(subAccountId: number): Promise<PromptTemplate[]> {
    return this.client.get<PromptTemplate[]>(`/admin/prompt-templates/subaccount/${subAccountId}`);
  }
  ...
  async create(data: CreatePromptTemplateDto): Promise<PromptTemplate> {
    ...
    const result = await this.client.post<PromptTemplate>('/admin/prompt-templates', data);
  ...
  async update(id: number, data: UpdatePromptTemplateDto): Promise<PromptTemplate> {
    ...
    const result = await this.client.patch<PromptTemplate>(`/admin/prompt-templates/${id}`, data);
  ...
  async activate(id: number, subAccountId: number): Promise<PromptTemplate> {
    return this.client.patch<PromptTemplate>(`/admin/prompt-templates/${id}/activate`, { subAccountId });
  }
