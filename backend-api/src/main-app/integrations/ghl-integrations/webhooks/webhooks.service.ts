...
export class WebhooksService {
  ...
  constructor(
    ...
    this.logger.log(
      `WebhooksService initialized with useDummyResponses=${this.useDummyResponses}`,
    );
  }
  ...
  async handleWebhook(webhookEventDto: WebhookEventDto) {
    ...
    switch (type) {
      ...
      default:
        ...
        return {
          status: 'unhandled',
          message: `Webhook type ${type} not implemented`,
        };
    }
  }
  ...
  private async handleContactCreated(payload: any) {
    ...
    try {
      ...
      this.logger.log(
        `Processing contact.created webhook for locationId: ${contactData.locationId}`,
      );
      ...
      if (!integration) {
        throw new HttpException(
          {
            status: 'error',
            message: `No integration found with GHL locationId (subaccount) ${contactData.locationId}`,
          },
      ...
      this.logger.log(
        `Found integration: ${integration.name} for subaccount: ${integration.subAccount.name}`,
      );
      ...
      if (!user) {
        throw new HttpException(
          {
            status: 'error',
            message: `No user found in subaccount ${integration.subAccount.name} for GHL locationId ${contactData.locationId}`,
          },
          HttpStatus.NOT_FOUND,
        );
      }
      ...
      this.logger.log(
        `Found user: ${user.name} (ID: ${user.id}) in subaccount: ${integration.subAccount.name}`,
      );
      ...
      if (strategy) {
        this.logger.log(
          `Found strategy: ${strategy.name} (ID: ${strategy.id}) for user: ${user.name}`,
        );
      } else {
        this.logger.warn(
          `No strategy found for user: ${user.name}, using default strategy ID: 1`,
        );
      }
      ...
      if (detectedTimezone) {
        this.logger.log(
          `Detected timezone: ${detectedTimezone} for contact ${contactData.id}`,
        );
      }
      ...
      this.logger.log(
        `Created lead: ${lead.name} (ID: ${lead.id}) for GHL contact: ${contactData.id}`,
      );
      ...
      this.logger.error(
        `Error processing contact.created webhook: ${error.message}`,
      );
      throw new HttpException(
  ...
  private async handleOutboundMessage(payload: any) {
    ...
    try {
      ...
      if (
        ...
        !allowedMessageTypes.includes(messageData.messageType)
      ) {
        return {
          status: 'ignored',
      ...
      if (!lead) {
        throw new HttpException(
          {
            status: 'error',
            message: `No lead found with GHL contactId ${messageData.contactId}`,
          },
          HttpStatus.NOT_FOUND,
        );
      }
      ...
      this.logger.error(
        `Error processing outbound.message webhook: ${error.message}`,
      );
  ...
  private async generateResponse(
    ...
    this.logger.log(
      `Generating response for lead ${leadId} to message: "${message}"`,
    );
  ...
  private async generateRealResponse(
    ...
    try {
      this.logger.log(
        `Calling AIReceptionistService.generateTextResponse for leadId=${leadId}`,
      );
      ...
      this.logger.log(
        `AIReceptionistService response generated successfully for leadId=${leadId}`,
      );
      return response;
    } catch (error) {
      this.logger.error(
        `Error generating real response for leadId=${leadId}: ${error.message}`,
      );
