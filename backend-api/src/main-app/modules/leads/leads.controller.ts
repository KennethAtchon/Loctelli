...
export class LeadsController {
  ...
  async create(@Body() createLeadDto: CreateLeadDto, @CurrentUser() user) {
    this.logger.log(
      `📝 Lead creation attempt by user: ${user.email} (ID: ${user.userId}, accountType: ${user.accountType})`,
    );
    ...
    try {
      ...
      if (isAdminAccount(user)) {
        ...
        if (!createLeadDto.subAccountId) {
          this.logger.warn(
            `Lead creation failed - subAccountId missing for admin user: ${user.email}`,
          );
          throw new HttpException(
            'subAccountId is required for lead creation',
            HttpStatus.BAD_REQUEST,
          );
        }
        this.logger.debug(
          `Admin creating lead for subAccountId: ${createLeadDto.subAccountId}`,
        );
        result = await this.leadsService.create(
          createLeadDto,
        ...
        this.logger.debug(
          `Regular user creating lead for subAccountId: ${user.subAccountId}`,
        );
        result = await this.leadsService.create(
          createLeadDto,
          user.subAccountId,
      ...
      this.logger.log(
        `✅ Lead created successfully by user: ${user.email} (Lead ID: ${result.id})`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Lead creation failed for user: ${user.email}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findAll(
    ...
    this.logger.log(
      `🔍 Lead search request by user: ${user.email} (ID: ${user.userId}, accountType: ${user.accountType})`,
    );
    this.logger.debug(
      `Search parameters: userId=${userId}, strategyId=${strategyId}, subAccountId=${subAccountId}`,
    );
    ...
    try {
      ...
      if (userId) {
        ...
        result = await this.leadsService.findByStrategyId(
          parsedStrategyId,
          user.userId,
          user.role,
        );
        ...
        if (isAdminAccount(user)) {
          ...
          if (parsedSubAccountId) {
            this.logger.debug(
              `Admin searching leads by subAccountId: ${parsedSubAccountId}`,
            );
            result =
              await this.leadsService.findAllBySubAccount(parsedSubAccountId);
          } else {
            // Return all leads from all SubAccounts owned by this admin
            this.logger.debug(
              `Admin searching all leads for adminId: ${user.userId}`,
            );
          ...
          this.logger.debug(
            `Regular user searching leads by subAccountId: ${user.subAccountId}`,
          );
          result = await this.leadsService.findAllBySubAccount(
            user.subAccountId,
          );
        }
      }
      ...
      this.logger.log(
        `✅ Lead search successful for user: ${user.email} - found ${result.length} leads`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Lead search failed for user: ${user.email}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findOne(@Param('id', ParseIntPipe) id: number, @CurrentUser() user) {
    this.logger.log(
      `🔍 Lead detail request for ID: ${id} by user: ${user.email} (ID: ${user.userId})`,
    );

    try {
      const result = await this.leadsService.findOne(
        id,
        user.userId,
        user.role,
      );
      this.logger.log(
        `✅ Lead detail retrieved successfully for ID: ${id} by user: ${user.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Lead detail retrieval failed for ID: ${id} by user: ${user.email}`,
        error.stack,
      );
      throw error;
    }
  ...
  async update(
    ...
    this.logger.log(
      `✏️ Lead update attempt for ID: ${id} by user: ${user.email} (ID: ${user.userId})`,
    );
    this.logger.debug(`Lead update data: ${JSON.stringify(updateLeadDto)}`);

    try {
      const result = await this.leadsService.update(
        id,
        updateLeadDto,
        user.userId,
        user.role,
      ...
      this.logger.log(
        `✅ Lead updated successfully for ID: ${id} by user: ${user.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Lead update failed for ID: ${id} by user: ${user.email}`,
        error.stack,
      );
      throw error;
    }
  ...
  async appendMessage(
    ...
    try {
      ...
      this.logger.error(
        `❌ Message append failed for lead ID: ${id}`,
        error.stack,
      );
      throw error;
    }
  ...
  async remove(@Param('id', ParseIntPipe) id: number, @CurrentUser() user) {
    this.logger.log(
      `🗑️ Lead deletion attempt for ID: ${id} by user: ${user.email} (ID: ${user.userId})`,
    );

    try {
      const result = await this.leadsService.remove(id, user.userId, user.role);
      this.logger.log(
        `✅ Lead deleted successfully for ID: ${id} by user: ${user.email}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Lead deletion failed for ID: ${id} by user: ${user.email}`,
        error.stack,
      );
      throw error;
    }
