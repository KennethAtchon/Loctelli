 ...
 * Using plain objects (no tool() wrapper) to avoid memory issues during type checking
 */
@Injectable()
export class BookingToolsVercel {
  private readonly logger = new Logger(BookingToolsVercel.name);

  private static readonly bookMeetingSchema = z.object({
    date: z
      .string()
    ...
    location: z
      .string()
      .describe('Meeting location (e.g., "Online", "Office", "Client Site")'),
  ...
  private static readonly checkAvailabilitySchema = z.object({
    date: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
    startTime: z
  ...
  createBookMeetingTool = (
    ...
    inputSchema: BookingToolsVercel.bookMeetingSchema,
    execute: async ({
      date,
      time,
      timezone,
  ...
  createCheckAvailabilityTool = (userId: number) => ({
    ...
    inputSchema: BookingToolsVercel.checkAvailabilitySchema,
