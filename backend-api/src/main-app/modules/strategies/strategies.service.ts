...
export class StrategiesService {
  ...
  async create(createStrategyDto: CreateStrategyDto, subAccountId: number) {
    this.logger.log(
      `Creating new strategy: ${createStrategyDto.name} for user ${createStrategyDto.regularUserId}, subAccount ${subAccountId}`,
    );

    // If no promptTemplateId is provided, get the active template as fallback
    if (!createStrategyDto.promptTemplateId) {
      this.logger.debug(
        'No promptTemplateId provided, attempting to find active template',
      );
      try {
        ...
        this.logger.warn(
          'No active template found, searching for any available template',
          error,
        );
        ...
        if (templates.length > 0) {
          ...
          this.logger.debug(
            `Using first available template: ${templates[0].id} (${templates.length} templates found)`,
          );
        } else {
      ...
      this.logger.debug(
        `Using provided promptTemplateId: ${createStrategyDto.promptTemplateId}`,
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `Strategy created successfully: ID ${createdStrategy.id}, name: ${createdStrategy.name}`,
      );
      return createdStrategy;
    } catch (error) {
      this.logger.error(
        `Failed to create strategy: ${createStrategyDto.name}`,
        error,
      );
      throw error;
    }
  ...
  async findAllBySubAccount(subAccountId: number) {
    ...
    try {
      ...
      this.logger.log(
        `Found ${strategies.length} strategies for subAccount ${subAccountId}`,
      );
      return strategies;
    } catch (error) {
      this.logger.error(
        `Failed to find strategies for subAccount ${subAccountId}`,
        error,
      );
      throw error;
    }
  ...
  async findAllByUser(userId: number) {
    ...
    try {
      ...
      this.logger.log(
        `Found ${strategies.length} strategies for user ${userId}`,
      );
      return strategies;
    } catch (error) {
  ...
  async findAllByAdmin(adminId: number) {
    ...
    try {
      ...
      this.logger.log(
        `Admin ${adminId} retrieved ${strategies.length} strategies`,
      );
      return strategies;
    } catch (error) {
      this.logger.error(
        `Failed to find strategies for admin ${adminId}`,
        error,
      );
      throw error;
    }
  ...
  async findOne(id: number, userId: number, userRole: string) {
    this.logger.debug(
      `Finding strategy ${id} for user ${userId} (role: ${userRole})`,
    );
    try {
      ...
      if (
        ...
        this.logger.warn(
          `User ${userId} attempted to access strategy ${id} owned by user ${strategy.regularUserId}`,
        );
        throw new ForbiddenException('Access denied');
      }

      this.logger.log(
        `Strategy ${id} found: ${strategy.name} (${strategy.leads?.length || 0} leads)`,
      );
      return strategy;
    } catch (error) {
      if (
  ...
  async findByUserId(userId: number) {
    ...
    try {
      ...
      this.logger.log(
        `Found ${strategies.length} strategies for userId ${userId}`,
      );
      return strategies;
    } catch (error) {
      this.logger.error(
        `Failed to find strategies for userId ${userId}`,
        error,
      );
      throw error;
    }
  ...
  async update(
    ...
    this.logger.log(
      `Updating strategy ${id} by user ${userId} (role: ${userRole})`,
    );
    ...
    if (
      ...
      this.logger.warn(
        `User ${userId} attempted to update strategy ${id} owned by user ${strategy.regularUserId}`,
      );
      throw new ForbiddenException('Access denied');
    }

    this.logger.debug(
      `Strategy ${id} found: ${strategy.name}, owned by user ${strategy.regularUserId}`,
    );

    // Check authorization for sensitive fields
    if (
      updateStrategyDto.regularUserId !== undefined &&
      !isAdminOrSuperAdmin(null, userRole)
    ) {
      this.logger.warn(
        `User ${userId} attempted to change strategy ${id} ownership`,
      );
      throw new ForbiddenException('Only admins can change strategy ownership');
    }

    if (
      updateStrategyDto.subAccountId !== undefined &&
      !isAdminOrSuperAdmin(null, userRole)
    ) {
      this.logger.warn(
        `User ${userId} attempted to change strategy ${id} subAccount`,
      );
      throw new ForbiddenException(
        'Only admins can change strategy subAccount',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `Strategy ${id} updated successfully: ${updatedStrategy.name}`,
      );
  ...
  async remove(id: number, userId: number, userRole: string) {
    this.logger.log(
      `Removing strategy ${id} by user ${userId} (role: ${userRole})`,
    );
    ...
    if (!strategy) {
      this.logger.warn(
        `Strategy ${id} not found for deletion by user ${userId}`,
      );
      throw new NotFoundException(`Strategy with ID ${id} not found`);
    }
    ...
    if (
      ...
      this.logger.warn(
        `User ${userId} attempted to delete strategy ${id} owned by user ${strategy.regularUserId}`,
      );
      throw new ForbiddenException('Access denied');
    }
    ...
    try {
      ...
      this.logger.log(
        `Strategy ${id} deleted successfully: ${deletedStrategy.name}`,
      );
  ...
  async duplicate(id: number, userId: number, userRole: string) {
    this.logger.log(
      `Duplicating strategy ${id} by user ${userId} (role: ${userRole})`,
    );
    ...
    if (!strategy) {
      this.logger.warn(
        `Strategy ${id} not found for duplication by user ${userId}`,
      );
      throw new NotFoundException(`Strategy with ID ${id} not found`);
    }
    ...
    if (
      ...
      this.logger.warn(
        `User ${userId} attempted to duplicate strategy ${id} owned by user ${strategy.regularUserId}`,
      );
      throw new ForbiddenException('Access denied');
    }

    this.logger.debug(
      `Duplicating strategy: ${strategy.name} (ID: ${id}), subAccount: ${strategy.subAccountId}, promptTemplate: ${strategy.promptTemplateId}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Strategy ${id} duplicated successfully: new ID ${duplicatedStrategy.id}, name: ${duplicatedStrategy.name}`,
      );
